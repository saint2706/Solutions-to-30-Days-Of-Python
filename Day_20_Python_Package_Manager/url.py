...
This script demonstrates using popular Python packages for data analysis
and API interaction, refactored for better testability and structure.
"""
...
def analyze_breed_metrics(breeds_data: List[Dict[str, Any]], metric: str, unit: str) -> Optional[Dict[str, float]]:
...
def analyze_breed_origins(breeds_data: List[Dict[str, Any]], top_n: int = 5) -> List[Tuple[str, int]]:
    """Analyzes the geographic distribution of cat breed origins."""
    origins = [breed['origin'] for breed in breeds_data if 'origin' in breed and breed['origin']]
...
def main():
    ...
    if cat_breeds:
        ...
        if weight_stats:
            print(f"Average Cat Weight: {weight_stats['mean']:.2f} {weight_stats['unit']}")

        lifespan_stats = analyze_breed_metrics(cat_breeds, 'life_span', 'years')
        if lifespan_stats:
            print(f"Average Cat Lifespan: {lifespan_stats['mean']:.2f} {lifespan_stats['unit']}")
