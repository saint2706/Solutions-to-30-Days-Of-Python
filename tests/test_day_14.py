...
def test_is_eligible_for_promotion():
    ...
    assert bl.is_eligible_for_promotion(years_of_service=4, performance_rating=4) is True
    assert bl.is_eligible_for_promotion(years_of_service=5, performance_rating=5) is True
    # Not eligible due to years
    assert bl.is_eligible_for_promotion(years_of_service=3, performance_rating=5) is False
    # Not eligible due to rating
    assert bl.is_eligible_for_promotion(years_of_service=4, performance_rating=3) is False
