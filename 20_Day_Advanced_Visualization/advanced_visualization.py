...
if not df.empty:
    ...
    sns.barplot(x='Product', y='Revenue', data=df, estimator=sum, errorbar=None, palette='viridis')
    ...
    plt.axhline(y=avg_revenue, color='red', linestyle='--', label=f'Avg Revenue (${avg_revenue:,.2f})')
    plt.legend() # Display the label for the line

    plt.tight_layout()
    # Save the figure before showing it
    # plt.savefig('customized_product_revenue.png', dpi=300)
    print("Displaying customized product revenue plot. Please close the plot window to continue.")
    plt.show()
    ...
    sns.histplot(df['Revenue'], bins=15, kde=True, ax=axes[1], color='green')
