...
def _update_mkdocs_nav(entries: list[str]) -> None:
    ...
    try:
        start = next(i for i, line in enumerate(contents) if line.strip() == "# AUTOGENERATED LESSON NAV START")
        end = next(i for i, line in enumerate(contents) if line.strip() == "# AUTOGENERATED LESSON NAV END")
    except StopIteration as exc:  # pragma: no cover - guard against manual edits
        raise RuntimeError("Could not locate AUTOGENERATED markers in mkdocs.yml") from exc
